...
if len(API_HASH) == 0:
    ...
    exit()
BOT_TOKEN = environ.get('BOT_TOKEN', '8533443457:AAHrH-q7FMfgdRblyTk-lZoW07XSjQXlVeI')
if len(BOT_TOKEN) == 0:
...
# Bot Admins
ADMINS = environ.get('ADMINS', '8587405600')
if len(ADMINS) == 0:
...
# Channels
INDEX_CHANNELS = [int(index_channels) if index_channels.startswith("-") else index_channels for index_channels in environ.get('INDEX_CHANNELS', '-1003370992067 -1003408919601 -1003760044053 -1003562255942 -1002658908912').split()]
if len(INDEX_CHANNELS) == 0:
    logger.info('INDEX_CHANNELS is empty')
LOG_CHANNEL = environ.get('LOG_CHANNEL', '-1003883965019')
...
# support group
SUPPORT_GROUP = environ.get('SUPPORT_GROUP', '-1002156622516')
...
# MongoDB information
DATA_DATABASE_URL = environ.get('DATA_DATABASE_URL', "mongodb+srv://adb75658_db_user:adb75658_db_user@cluster0.ypdgftp.mongodb.net/?appName=Cluster0")
if len(DATA_DATABASE_URL) == 0:
    logger.error('DATA_DATABASE_URL is missing, exiting now')
    exit()
FILES_DATABASE_URL = environ.get('FILES_DATABASE_URL', "mongodb+srv://DQ_DB2:DQ_DB22@cluster0.gf6cxly.mongodb.net/?appName=Cluster0")
if len(FILES_DATABASE_URL) == 0:
    logger.error('FILES_DATABASE_URL is missing, exiting now')
    exit()
SECOND_FILES_DATABASE_URL = environ.get('SECOND_FILES_DATABASE_URL', "mongodb+srv://DQ_DB1:DQ_DB11@cluster0.1pxwbbu.mongodb.net/?appName=Cluster0")
...
BIN_CHANNEL = environ.get("BIN_CHANNEL", "-1003883965019")
if len(BIN_CHANNEL) == 0:
    logger.error('BIN_CHANNEL is missing, exiting now')
    exit()
else:
    BIN_CHANNEL = int(BIN_CHANNEL)
URL = environ.get("URL", "unpleasant-amabel-asmoviegroup-20d706ea.koyeb.app/")
if len(URL) == 0:
